...
import { ROLE_PRESETS, encodePermissions, isStaffRole, sanitisePermissions, } from './permissions.js';
/**
 ...
 */
export const onStaffWritten = onDocumentWritten('tenants/{tenantId}/staff/{uid}', async (event) => {
    ...
    if (before && !permissionsChanged(before, after) && before.active === after.active)
        return;
 ...
 */
async function touchClaims(tenantId, uid) {
    await db()
...
function staffCol(tenantId) {
    return db().collection(`tenants/${tenantId}/staff`);
}
async function countActiveOwners(tenantId, excludingUid) {
...
export const createStaff = onCall(async (request) => {
    ...
    const role = isStaffRole(data.role) ? data.role : 'staff';
    ...
    const permissions = sanitisePermissions(data.permissions, ROLE_PRESETS[role]);
    const auth = getAuth();
...
export const updateStaff = onCall(async (request) => {
    ...
    if (wantsRoleChange || wantsPermsChange) {
        ...
        const role = isStaffRole(data.role) ? data.role : current.role;
        if (role === 'owner') {
            throw new HttpsError('invalid-argument', 'a tenant has exactly one owner');
        }
        ...
        patch.role = role;
        patch.permissions = sanitisePermissions(wantsPermsChange ? data.permissions : current.permissions, ROLE_PRESETS[role]);
    }
